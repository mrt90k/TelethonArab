...
async def _(event):
    if (event.is_private or event.is_group):
        return await edit_or_reply(event, "عذرا المشاركه تشتغل فقط على القنوات .")
    ...
    if str(hel_).startswith("-100"):
        ...
        iq = hel_
    if not iq.isdigit():
        return await edit_or_reply(event, "**رجاء قم بوضع ايدي القناه بجانب الأمر وفي حاله لاتعرف كيفيه ستعمل الامر هناك شرح للأمر في هذا القناه ابحث عنه  @l3ll3 !!**")
    if is_post(iq , event.chat_id):
        return await edit_or_reply(event, "هذا القناه بالفعل مفعل بها امر المشاركه")
    add_post(iq, event.chat_id)
    await edit_or_reply(event, f"**تم بدء امر المشاركه لهذا القناه :** `{hel_}`")
...
async def _(event):
    ...
    if not is_post(iq, event.chat_id):
        return await edit_or_reply(event, " هذا القناه ليس مفعل  بها امر المشاركه.")
    remove_post(iq, event.chat_id)
    await edit_or_reply(event, f"** تم اطفاء امر المشاركه بنجاح ** `{hel_}`")
